...
type GameStats = {
  ...
  coloringPixels: number,
  memoryBestScore: number | null,
...
export default function GamesPage() {
  ...
  const updateBreathingTime = (seconds: number) => {
    const updatedStats = {
      ...stats,
  ...
  const updateColoringPixels = (pixels: number) => {
    const updatedStats = {
      ...stats,
  ...
  const updateMemoryStats = (moves: number) => {
    const updatedStats = {
      ...stats,
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-green-50">
      <div className="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-primary">Games Corner</h1>
...
function BreathingExercise({ onSessionComplete }: { onSessionComplete: (seconds: number) => void }) {
  const [isActive, setIsActive] = useState(false)
  const [breathingRate, setBreathingRate] = useState(4) // in seconds
  const [currentPhase, setCurrentPhase] = useState<"in" | "hold" | "out" | "rest">("in")
  ...
  // const totalCycleTime = phaseDurations.in + phaseDurations.hold + phaseDurations.out + phaseDurations.rest
  ...
  useEffect(() => {
    ...
    if (isActive) {
      let phaseTime = 0
      let currentPhase: "in" | "hold" | "out" | "rest" = "in"
          ...
          <div className="p-4 bg-gray-50 rounded-lg border border-gray-100 space-y-4">
            <div className="space-y-2">
              ...
              <Slider
                ...
                disabled={isActive}
                className="py-1 [&>.SliderTrack]:bg-black"
              />
...
function ColoringGame({ onPixelsColored }: { onPixelsColored: (pixels: number) => void }) {
  const [selectedColor, setSelectedColor] = useState("#4f46e5")
  const [pixels, setPixels] = useState(Array(400).fill("#ffffff"))
  const [coloredPixels, setColoredPixels] = useState(0)
  const [selectedTool, setSelectedTool] = useState<"brush" | "fill">("brush")
  ...
  const [activePalette, setActivePalette] = useState<keyof typeof palettes | "default">("default")

  const displayColors = activePalette === "default" ? colors : palettes[activePalette as keyof typeof palettes]
...
function MemoryGame({ onGameComplete }: { onGameComplete: (moves: number) => void }) {
  ...
  function generateCards(difficulty: "easy" | "medium" | "hard") {
    const levelConfig = {
